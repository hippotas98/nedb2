  ...
  for(let iter = 0;iter<25;++iter){
    executeAsap(() =>{
      ...
      for(let j = 1; j<1000000;++j){
        users.push({
          Cx: j + iter*1000000 + 75000000, //Chinh sau moi lan chay
          name: {}
        })
      }
...
const insertSmallerDocs = (d, s, e, profiler, cb)=> {
  ...
  function runFrom(i) {
  if (i >= e || e > 25000000) {   // Finished
    if(i >= 25000000) {
     var opsPerSecond = Math.floor(1000* 25000000/ profiler.elapsedSinceLastStep());
      console.log("===== RESULT (insert) ===== " + opsPerSecond + " ops/s");
